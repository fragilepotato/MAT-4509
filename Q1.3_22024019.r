...
# Load data
data <- read_excel("D:/vscode/R/United Airlines Aircraft Operating Statistics- Cost Per Block Hour (Unadjusted).xls", range="b2:w158")
...
get_row <- function(row, data) {
  na.omit(as.numeric(data[row, -1]))[1:19]
}
...
create_and_save_table <- function(data, fleet_name) {
  ...
  # Save plot as PNG
  png_filename <- paste0("D:/vscode/R/Charts", fleet_name, "_wages_distribution.png")
